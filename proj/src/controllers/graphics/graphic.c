...
int set_frame_buffer(uint16_t mode) {
  ...
  if (video_mem == MAP_FAILED || back_buf == NULL) {
...
int vg_draw_pixel(uint16_t x, uint16_t y, uint32_t color) {
  ...
  if (memcpy(&back_buf[idx], &color, bytes_per_pixel) == NULL)
    return 1;
  return 0;
}
...
int clear_area(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint32_t color) {
  ...
  for (uint16_t i = y; i < y + height; i++) {
    for (uint16_t j = x; j < x + width; j++) {
      if (vg_draw_pixel(j, i, normalized_color) != 0)
        return 1;
    }
  }
  return 0;
...
int norm_color(uint32_t color, uint32_t *n_color) {
  if (mode_info.BitsPerPixel == 32) {
    ...
    *n_color = color & (BIT(mode_info.BitsPerPixel) - 1);
  }
  return 0;
}
...
int(print_xpm)(xpm_map_t xpm, uint16_t x, uint16_t y) {
  ...
  for (int h = 0; h < image.height; h++) {
    for (int w = 0; w < image.width; w++) {
      if (vg_draw_pixel(w + x, h + y, colors[h * image.width + w]) != 0)
        return 1;
    }
  }
  return 0;
